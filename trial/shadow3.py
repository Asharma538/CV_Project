...
# img_shadow = removeShadow("Images\image1.jpg")
# cv2.imshow('',np.uint8(img_shadow))
# #   folder = './Images/'
...
def fineTune(img, L, G, final):
    ...
    thresh = threshold_otsu(cv2.cvtColor(L,cv2.COLOR_BGR2GRAY))
...
def removeShadow(filename):
    ...
    final = get_FinalImg(img, L, G) # show the input vs output
    tuned = fineTune(img, L, G, final) # show effects of tuning
    cv2.imshow('hello',tuned)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

removeShadow('./Images/image2.jpeg')